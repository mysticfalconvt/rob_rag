...
export class PaperlessClient {
    ...
    async getCorrespondentName(id: number | null): Promise<string | null> {
        ...
        // Fetch from API
        try {
            const correspondent = await this.makeRequest<PaperlessCorrespondent>(`/correspondents/${id}/`);
...
export async function getPaperlessClient(): Promise<PaperlessClient | null> {
    try {
        ...
        if (!settings || !settings.paperlessEnabled || !settings.paperlessUrl || !settings.paperlessApiToken) {
            return null;
        }
